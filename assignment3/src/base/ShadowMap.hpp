...
class ShadowMapContext {
   public:
    ShadowMapContext() : m_depthRenderbuffer(0), m_framebuffer(0), m_resolution(256, 256) {}
...
class LightSource {
   public:
    ...
    void draw(const Mat4f& worldToCamera,
              const Mat4f& projection,
              bool show_axis = false,
              bool show_frame = false,
              bool show_square = false);  // for visualization
    ...
    void renderShadowedScene(GLContext* gl,
                             MeshWithColors* scene,
                             const Mat4f& worldToCamera,
                             const Mat4f& projection,
                             bool fromLight = false);
    void renderShadowMap(FW::GLContext* gl,
                         MeshWithColors* scene,
                         ShadowMapContext* sm,
                         bool debug = false);
    ...
    void sampleEmittedRays(int num,
