...
bool fileExists(std::string fileName) { return std::ifstream(fileName).good(); }
...
App::App(std::vector<std::string>& cmd_args)
    : m_commonCtrl(CommonControls::Feature_Default & ~CommonControls::Feature_RepaintOnF5),
      m_cameraCtrl(&m_commonCtrl,
                   CameraControls::Feature_Default | CameraControls::Feature_StereoControls),
    ...
    m_commonCtrl.addButton(
        (S32*)&m_action, Action_EncodeCameraSignature, FW_KEY_NONE, "Encode camera signature");
    m_commonCtrl.addButton(
        (S32*)&m_action, Action_DecodeCameraSignature, FW_KEY_NONE, "Decode camera signature...");
    m_window.addListener(&m_cameraCtrl);
    m_commonCtrl.addSeparator();

    m_commonCtrl.addButton((S32*)&m_action, Action_NormalizeScale, FW_KEY_NONE, "Normalize scale");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_FlipXY,                  FW_KEY_NONE, "Flip
    //    X/Y"); m_commonCtrl.addButton((S32*)&m_action, Action_FlipYZ, FW_KEY_NONE,    "Flip Y/Z");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_FlipZ,                   FW_KEY_NONE, "Flip
    //    Z");
    m_commonCtrl.addSeparator();

    m_commonCtrl.addButton(
        (S32*)&m_action, Action_NormalizeNormals, FW_KEY_NONE, "Normalize normals");
    m_commonCtrl.addButton((S32*)&m_action, Action_FlipNormals, FW_KEY_NONE, "Flip normals");
    m_commonCtrl.addButton(
        (S32*)&m_action, Action_RecomputeNormals, FW_KEY_NONE, "Recompute normals");
    m_commonCtrl.addSeparator();

    m_commonCtrl.addToggle(
        (S32*)&m_cullMode, CullMode_None, FW_KEY_NONE, "Disable backface culling");
    m_commonCtrl.addToggle((S32*)&m_cullMode, CullMode_CW, FW_KEY_NONE, "Cull clockwise faces");
    m_commonCtrl.addToggle(
        (S32*)&m_cullMode, CullMode_CCW, FW_KEY_NONE, "Cull counter-clockwise faces");
    ...
    //    "Remove unused materials, denegerate triangles, and unreferenced vertices");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_CollapseVertices,        FW_KEY_NONE,
    //    "Collapse duplicate vertices"); m_commonCtrl.addButton((S32*)&m_action,
    //    Action_DupVertsPerSubmesh,      FW_KEY_NONE,    "Duplicate vertices shared between
    //    multiple materials"); m_commonCtrl.addButton((S32*)&m_action, Action_FixMaterialColors,
    //    FW_KEY_NONE,    "Override material colors with average over texels");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_DownscaleTextures,       FW_KEY_NONE,
    ...
    m_commonCtrl.addToggle((S32*)&m_action,
                           ...
                           FW_KEY_F1,
                           "Render with spherical harmonics (F1)");
    m_commonCtrl.addButton(
        (S32*)&m_action, Action_ComputeRadiosity, FW_KEY_ENTER, "Compute Radiosity (ENTER)");
    m_commonCtrl.addButton((S32*)&m_action,
                           Action_PlaceLightSourceAtCamera,
                           FW_KEY_SPACE,
                           "Place light at camera (SPACE)");
    m_commonCtrl.addButton(
        (S32*)&m_action, Action_LoadRadiosity, FW_KEY_NONE, "Load radiosity solution");
    m_commonCtrl.addButton(
        (S32*)&m_action, Action_SaveRadiosity, FW_KEY_NONE, "Save radiosity solution");

    m_commonCtrl.beginSliderStack();
    m_commonCtrl.addSlider(
        &m_numDirectRays, 1, 1024, true, FW_KEY_NONE, FW_KEY_NONE, "Direct lighting rays= %d");
    m_commonCtrl.addSlider(&m_numHemisphereRays,
                           1,
                           4096,
                           true,
                           FW_KEY_NONE,
                           FW_KEY_NONE,
                           "Diffuse hemisphere rays= %d");
    m_commonCtrl.addSlider(
        &m_numBounces, 0, 8, false, FW_KEY_NONE, FW_KEY_NONE, "Number of indirect bounces= %d");
    m_commonCtrl.addSlider(
        &m_lightSize, 0.01f, 2.0f, false, FW_KEY_NONE, FW_KEY_NONE, "Light source area= %f");
    m_commonCtrl.endSliderStack();
    m_commonCtrl.beginSliderStack();
    m_commonCtrl.addSlider(&m_toneMapWhite,
                           0.05f,
                           10.0f,
                           true,
                           FW_KEY_NONE,
                           FW_KEY_NONE,
                           "Tone mapping parameter 1= %f");
    m_commonCtrl.addSlider(&m_toneMapBoost,
                           0.05f,
                           10.0f,
                           true,
                           FW_KEY_NONE,
                           FW_KEY_NONE,
                           "Tone mapping parameter 2= %f");
    m_commonCtrl.endSliderStack();
...
int find_argument(std::string needle, std::vector<std::string> haystack) {
    for (unsigned j = 0; j < haystack.size(); ++j)
        if (!haystack[j].compare(needle)) return j;

    return -1;
}

void App::process_args(std::vector<std::string>& args) {
    // all of the possible cmd arguments and the corresponding enums (enum value is the index of the
    ...
    const std::vector<std::string> builder_names = {
        "none", "sah", "object_median", "spatial_median", "linear"};
    enum builder_type {
        builder_not_found = -1,
        builder_None = 0,
        builder_SAH = 1,
        ...
        switch (cmd) {
            ...
            case builder: {
                ...
                if (type == builder_not_found) {
                    type = builder_SAH;
                    std::cout << "BVH builder not recognized, using Surface Area Heuristic"
                              << std::endl;
                    break;
                }
    ...
    if (m_settings.batch_render) m_settings.use_textures = false;
}
...
bool App::handleEvent(const Window::Event& ev) {
    ...
    switch (action) {
        ...
        case Action_LoadMesh:
            name = m_window.showFileLoadDialog("Load mesh or state",
                                               getMeshImportFilter() + ",dat:State file");
            if (name.getLength())
        ...
        case Action_ReloadMesh:
            if (m_meshFileName.getLength()) loadMesh(m_meshFileName);
            break;

        case Action_SaveMesh:
            name = m_window.showFileSaveDialog("Save mesh", getMeshExportFilter());
            if (name.getLength()) saveMesh(name);
            break;
        ...
        case Action_NormalizeScale:
            if (m_mesh) {
                ...
                m_mesh->xform(Mat4f::scale(Vec3f(2.0f / (hi - lo).max())) *
        ...
        case Action_NormalizeNormals:
            if (m_mesh) m_mesh->xformNormals(mat.getXYZ(), true);
            break;

        case Action_FlipNormals:
            mat = -mat;
            if (m_mesh) m_mesh->xformNormals(mat.getXYZ(), false);
            break;

        case Action_RecomputeNormals:
            if (m_mesh) m_mesh->recomputeNormals();
            break;

        case Action_FlipTriangles:
            if (m_mesh) m_mesh->flipTriangles();
            break;

        case Action_CleanMesh:
            if (m_mesh) m_mesh->clean();
            break;

        case Action_CollapseVertices:
            if (m_mesh) m_mesh->collapseVertices();
            break;

        case Action_DupVertsPerSubmesh:
            if (m_mesh) m_mesh->dupVertsPerSubmesh();
            break;

        case Action_FixMaterialColors:
            if (m_mesh) m_mesh->fixMaterialColors();
            break;

        case Action_DownscaleTextures:
            if (m_mesh) downscaleTextures(m_mesh.get());
            break;

        case Action_ChopBehindNear:
            if (m_mesh) {
                Mat4f worldToClip =
                    m_cameraCtrl.getCameraToClip() * m_cameraCtrl.getWorldToCamera();
        ...
        case Action_ComputeRadiosity:
            m_radiosity->startRadiosityProcess(m_mesh.get(),
                                               m_areaLight.get(),
                                               m_rt.get(),
                                               m_numBounces,
                                               m_numDirectRays,
    ...
    if (ev.type == Window::EventType_Paint) renderFrame(m_window.getGL());
    m_window.repaint();
    return false;
}
...
void App::readState(StateDump& d) {
    ...
    m_lightSize =
        m_areaLight->getSize().x;  // dirty; doesn't allow for rectangular lights, only square. TODO
...
void App::renderFrame(GLContext* gl) {
    ...
    Mat4f projection =
        gl->xformFitToView(Vec2f(-1.0f, -1.0f), Vec2f(2.0f, 2.0f)) * m_cameraCtrl.getCameraToClip();
    ...
    if (m_shChanged || (m_radiosity->isRunning() && m_updateClock.getElapsed() > 0.5f)) {
        ...
        if (m_radiosity->updateMeshColors(
                m_sphericalResults1, m_sphericalResults2, m_sphericalResults3, m_enableSH)) {
            if (m_radiosity->isRunning()) m_radiosity->checkFinish();

            glBindBuffer(GL_ARRAY_BUFFER, m_sphericalBuffer1);

            sendTangents(&m_sphericalResults3);

            GLuint spherical1Location =
                       glGetAttribLocation(prog->getHandle(), "sphericalHarmonic1"),
                   spherical2Location =
                       glGetAttribLocation(prog->getHandle(), "sphericalHarmonic2");

            if (spherical1Location != -1) {
                glBindBuffer(GL_ARRAY_BUFFER, m_sphericalBuffer1);
                glBufferData(GL_ARRAY_BUFFER,
                             m_sphericalResults1.size() * sizeof(Vec4f),
                             m_sphericalResults1.data(),
                             GL_STATIC_DRAW);
            ...
            if (spherical2Location != -1) {
                ...
                glBufferData(GL_ARRAY_BUFFER,
                             m_sphericalResults2.size() * sizeof(Vec4f),
                             m_sphericalResults2.data(),
                             GL_STATIC_DRAW);
...
void App::loadMesh(const String& fileName) {
    ...
    if (restoreError(oldError)) {
        m_commonCtrl.message(
            sprintf("Error while loading '%s': %s", fileName.getPtr(), newError.getPtr()));
        return;
    }
    ...
    for (S32 i = 0; i < m_mesh->numVertices(); ++i) m_mesh->mutableVertex(i).c = Vec3f(1, 1, 1);
...
void App::saveMesh(const String& fileName) {
    ...
    if (restoreError(oldError)) {
        m_commonCtrl.message(
            sprintf("Error while saving '%s': %s", fileName.getPtr(), newError.getPtr()));
        return;
    }
...
void App::constructTracer() {
    ...
    for (int i = 0; i < m_mesh->numSubmeshes(); ++i) {
        ...
        for (int j = 0; j < idx.getSize(); ++j) {
            const VertexPNTC &v0 = m_mesh->vertex(idx[j][0]), &v1 = m_mesh->vertex(idx[j][1]),
            ...
            // EXTRA: generate tangent space for triangle t here (you can probably reuse code from
            // assn1 if you implemented tangent space normal mapping)
            for (int k = 0; k < 3; ++k) {
    ...
    for (auto& tangent : m_tangents) tangent.normalize();
    for (auto& bitangent : m_bitangents) bitangent.normalize();
    ...
    if (tryLoadHierarchy) {
        ...
        if (fileExists(hierarchyCacheFile.getPtr())) {
            ...
            int build_time = (int)((stop.QuadPart - start.QuadPart) * 1000.0 /
        ...
        m_results.build_time = (int)((stop.QuadPart - start.QuadPart) * 1000.0 /
                                     frequency.QuadPart);  // Get timer result in milliseconds
...
// This method sends the tangent and bitangent arrays to the GPU. One component of the spherical
// harmonics is also added as the fourth component of the tangent.
void App::sendTangents(std::vector<float>* sphericalCoefficients) {
    ...
    if (sphericalCoefficients == nullptr)
        for (int i = 0; i < m_tangents.size(); ++i) tangents[i] = Vec4f(m_tangents[i], .0f);
    else
    ...
    if (tangentLocation != -1) {
        ...
        glBufferData(
            GL_ARRAY_BUFFER, tangents.size() * sizeof(Vec4f), tangents.data(), GL_STATIC_DRAW);
    ...
    if (bitangentLocation != -1) {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     m_bitangents.size() * sizeof(Vec3f),
                     m_bitangents.data(),
                     GL_STATIC_DRAW);
...
void App::setupShaders(GLContext* gl) {
    GLContext::Program* prog = new GLContext::Program(
        "#version 120\n" FW_GL_SHADER_SOURCE(
            uniform mat4 posToClip; uniform mat4 posToCamera; uniform mat3 normalToCamera;
            attribute vec3 positionAttrib;
            attribute vec3 normalAttrib;
            attribute vec4
                vcolorAttrib;  // Workaround. "colorAttrib" appears to confuse certain ATI drivers.
            attribute vec2 texCoordAttrib;
            attribute vec4 sphericalHarmonic1;
            attribute vec4 sphericalHarmonic2;
            attribute vec4 tangent;  // the fourth component is spherical harmonics data, both the
                                     // tangent and bitangent are three-dimensional.
            ...
            void main() {
                // the coefficients have to be packed to keep the amount of attributes down - ugly
                // but necessary (and not vital to understand)
        ...
        "#version 120\n" FW_GL_SHADER_SOURCE(
            uniform bool hasNormals; uniform bool hasDiffuseTexture; uniform bool hasAlphaTexture;
            ...
            void main() {
                ...
                if (hasAlphaTexture) diffuseColor.a = texture2D(alphaSampler, texCoordVarying).g;
...
void App::loadRadiosity(const String& fileName) {
    ...
    String savedMD5;
    f >> savedMD5;
    if (savedMD5 != md5) printf("Radiosity was computed for another scene!");
    ...
    for (S32 i = 0; i < m_mesh->numVertices(); ++i) m_mesh->mutableVertex(i).c = v[i];
}
...
void App::saveRadiosity(const String& fileName) {
    ...
    std::vector<Vec3f> v;
    v.resize(m_mesh->numVertices());
    for (S32 i = 0; i < m_mesh->numVertices(); ++i) v[i] = m_mesh->vertex(i).c;
...
void FW::init(std::vector<std::string>& args) { new App(args); }
