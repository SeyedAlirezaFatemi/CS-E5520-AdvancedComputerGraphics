...
App::App(std::vector<std::string>& cmd_args)
    ...
    //    "Remove unused materials, denegerate triangles, and unreferenced
    //    vertices"); m_commonCtrl.addButton((S32*)&m_action,
    //    Action_CollapseVertices,        FW_KEY_NONE,    "Collapse duplicate
    //    vertices"); m_commonCtrl.addButton((S32*)&m_action,
    //    Action_DupVertsPerSubmesh,      FW_KEY_NONE,    "Duplicate vertices
    //    shared between multiple materials");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_FixMaterialColors,
    //    FW_KEY_NONE,    "Override material colors with average over texels");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_DownscaleTextures,
    //    FW_KEY_NONE,    "Downscale textures by 2x");
    //    m_commonCtrl.addButton((S32*)&m_action, Action_ChopBehindNear,
    //    FW_KEY_NONE,    "Chop triangles behind near plane");
    //    m_commonCtrl.addSeparator();
    ...
    if (!m_settings.batch_render) {
        ...
        if (m_settings.output_images) {
            FW::exportImage(
                std::string("images/" + m_results.state_name + ".png").c_str(),
                m_rtImage.get());
        }

        // :: alone refers to the local anonymous namespace (present if no other
        // specified)
        bool created = !(::fileExists(cmd_args[2]));

        std::ofstream result(cmd_args[2],
                             std::ios_base::out | std::ios_base::app);

        if (created)
            result << "set_name scene_name state_name build_time(ms) "
                      "trace_time(ms) "
                      "ray_count"
                   << std::endl;

        result << cmd_args[3] << " " << m_results.scene_name << " "
               << m_results.state_name << " " << m_results.build_time << " "
               << m_results.trace_time << " " << m_results.rayCount
               << std::endl;
    ...
    for (unsigned i = 0; i < args.size(); ++i) {
        ...
        switch (cmd) {
            ...
            case builder: {
                ++i;
                builder_type type =
                    builder_type(find_argument(args[i], builder_names));

                if (type == builder_not_found) {
                    type = builder_SAH;
                    std::cout << "BVH builder not recognized, using Surface "
                                 "Area Heuristic"
                              << std::endl;
                    break;
                }
            ...
            default:
                if (args[i][0] == '-')
                    std::cout << "argument \"" << args[i] << "\" not found!"
                              << std::endl;
        }
    }
...
bool App::handleEvent(const Window::Event& ev) {
    ...
    switch (action) {
        ...
        case Action_LoadMesh:
            name = m_window.showFileLoadDialog(
                "Load mesh or state",
        ...
        case Action_SaveMesh:
            name =
                m_window.showFileSaveDialog("Save mesh", getMeshExportFilter());
        ...
        case Action_ChopBehindNear:
            if (m_mesh) {
                Mat4f worldToClip = m_cameraCtrl.getCameraToClip() *
                                    m_cameraCtrl.getWorldToCamera();
...
void App::renderFrame(GLContext* gl) {
    ...
    if (m_showRTImage) {
        ...
        m_commonCtrl.message(
            sprintf("%.2f Mrays/sec",
                    m_renderer->getRaysPerSecond() / 1000000.0f),
            "rayStats");
    } else {
        ...
        Mat4f projection =
            gl->xformFitToView(Vec2f(-1.0f, -1.0f), Vec2f(2.0f, 2.0f)) *
            m_cameraCtrl.getCameraToClip();
...
void App::blitRttToScreen(GLContext* gl) {
    ...
    if (!prog) {
        prog = new GLContext::Program(
            ...
            FW_GL_SHADER_SOURCE(
                ...
                void main() {
                    vec3 color =
                        texture2D(texSampler, vec2(uv.x, 1.0 - uv.y)).rgb;
                    color *= tonemapBoost;
                    float L =
                        0.2126 * color.x + 0.7152 * color.y + 0.0722 * color.z;
                    color *=
                        (1.0 + L / reinhardLWhite / reinhardLWhite) / (1.0 + L);
    ...
    glGetIntegerv(GL_TEXTURE_BINDING_2D,
                  &oldTex);  // get currently bound texture so texture bind
                             // state can remain unchanged
...
void App::loadMesh(const String& fileName) {
    // find the scene name; the file path without preceding folders and file
    // extension
    ...
    m_window.showModalMessage(
        sprintf("Loading mesh from '%s'...", fileName.getPtr()));
...
void App::saveMesh(const String& fileName) {
    ...
    m_window.showModalMessage(
        sprintf("Saving mesh to '%s'...", fileName.getPtr()));
    String oldError = clearError();
...
// This function iterates over all the "sub-meshes" (parts of the object with
// different materials), heaps all the vertices and triangles together, and
// calls the BVH constructor. It is the responsibility of the tree to free the
// data when deleted. This functionality is _not_ part of the RayTracer class in
// order to keep it separate from the specifics of the Mesh class.
...
void App::downscaleTextures(MeshBase* mesh) {
    ...
    for (int submeshIdx = 0; submeshIdx < mesh->numSubmeshes(); submeshIdx++)
        for (int textureIdx = 0; textureIdx < MeshBase::TextureType_Max;
             textureIdx++) {
            ...
            if (tex.exists()) {
                ...
                if (!hash.contains(orig)) {
                    ...
                    hash.add(orig,
                             (scaled) ? Texture(scaled, tex.getID()) : tex);
                }
                tex = hash.get(orig);
            }
        }
...
void App::chopBehindPlane(MeshBase* mesh, const Vec4f& pleq) {
    ...
    for (int submeshIdx = 0; submeshIdx < mesh->numSubmeshes(); submeshIdx++) {
        ...
        for (int triIn = 0; triIn < inds.getSize(); triIn++) {
            if (dot(mesh->getVertexAttrib(inds[triIn].x, posAttrib), pleq) >=
                    0.0f ||
                dot(mesh->getVertexAttrib(inds[triIn].y, posAttrib), pleq) >=
                    0.0f ||
                dot(mesh->getVertexAttrib(inds[triIn].z, posAttrib), pleq) >=
                    0.0f) {
